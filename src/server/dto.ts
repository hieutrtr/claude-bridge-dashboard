...
// P3-T8 — wire shape returned by `schedules.runs`. Curated subset of
// the daemon `tasks` table — column set chosen to render the run-
// history drawer self-sufficiently (status badge + cost/duration +
// link to /tasks/[id]). The schedule's metadata (`scheduleName`,
// `agentName`) is echoed on the envelope so the drawer header doesn't
// need a second round-trip.
//
// `prompt` is intentionally NOT on the wire — already visible on the
// `/schedules` row tooltip and the `/tasks/[id]` detail page; no need
// to re-echo it here. Drawer body is just the per-run columns.
//
// `agentName` is the *schedule's* agent, not a per-task projection.
// Tasks generated by the schedule all share the same agent (the
// scheduler dispatches against `schedule.agent_name` exclusively).
